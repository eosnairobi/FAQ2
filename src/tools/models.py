...
def suggested_tool_logo_directory_path(instance, filename):
    # image will be uploaded to MEDIA_ROOT/Tools/<ToolName>/<filename>
    return 'SuggestedTools/{0}/{1}'.format(instance.name, filename)
...
class SuggestedTool(models.Model):
    ...
    description = models.CharField(max_length=1000)
    image = models.ImageField(blank=True, null=True,
                              upload_to=suggested_tool_logo_directory_path)
    url = models.URLField(blank=True, null=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, null=True)
